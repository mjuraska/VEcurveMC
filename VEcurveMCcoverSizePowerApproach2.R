# for each MC iteration, this code computes indicators of CI coverage, and indicators of rejection of the null hypothesis in tests of H^1_0, H^2_0, and H^4_0 under validity of
# both the null and an alternative hypothesis
# this code uses a nonparametric kernel density estimator
...
# betas for coverage, power of the test of H^1_0 and H^2_0, and size of the test of H^4_0
# these betas are also used in one population for power of the test of H^4_0
...
# betas for size of the test of H^1_0 and H^2_0
...
# betas in the other population for power of H^4_0
...
for (i in 1:length(pi)){
  inferenceVE <- lapply(1:nMC, function(seed, s1grid, trueVEcurve, n, beta, betaSizeT1, betaPowerT3, pi, truncateMarker, nBoot){ 
    getInferenceVE2(s1grid, trueVEcurve, n, beta, betaSizeT1, betaPowerT3, pi, truncateMarker, seed, nBoot) }, s1grid=s1, trueVEcurve=trueVEcurve, n=n, beta=beta, 
    betaSizeT1=betaSizeT1, betaPowerT3=betaPowerT3, pi=pi[i], truncateMarker=truncateMarker, nBoot=nBoot)
  
  save(inferenceVE, file=file.path(outDir, paste0("VE_coverSizePower_RoyBose_ns.df=3_nMC=",nMC,"_N=",n,"_nBoot=",nBoot,"_truncateMarker=",truncateMarker,"_pi=",pi[i],".RData")))
}
