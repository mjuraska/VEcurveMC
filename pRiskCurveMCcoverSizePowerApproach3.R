...
# true risk curves evaluated on the grid
trueRiskCurves <- trueRisk(s1, beta[1], beta[2], beta[3], beta[4], 2, 1, 3, 1, 0.7, scenario="A2", threshold=1.5)
...
for (i in 1:length(pi)){
  inferenceMCEP <- lapply(1:nMC, function(seed, s1grid, trueRiskCurves, n, beta, betaSizeT1, betaPowerT3, pi, truncateMarker, nBoot){ 
    getPinferenceMCEPcurve2(s1grid, trueRiskCurves, n, beta, betaSizeT1, betaPowerT3, pi, truncateMarker, seed, nBoot) }, s1grid=s1, trueRiskCurves=trueRiskCurves, n=n, beta=beta, 
    betaSizeT1=betaSizeT1, betaPowerT3=betaPowerT3, pi=pi[i], truncateMarker=truncateMarker, nBoot=nBoot)
  
  save(inferenceMCEP, file=file.path(outDir, paste0("pAddDiff_coverSizePowerApproach3_ns.df=3_nMC=",nMC,"_N=",n,"_nBoot=",nBoot,"_truncateMarker=",truncateMarker,"_pi=",pi[i],".RData")))
}
