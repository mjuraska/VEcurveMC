...
for (i in 1:length(pi)){
  ...
  coverVE <- sapply(1:nMC, function(seed, s1grid, trueVEcurve, n, beta, pi, truncateMarker, nBoot){ getCoverVE(s1grid, trueVEcurve, n, beta, pi, truncateMarker, seed, nBoot) }, s1grid=s1, trueVEcurve=trueVEcurve, n=n, beta=beta, pi=pi[i], truncateMarker=truncateMarker, nBoot=nBoot)
  write.table(coverVE, file=file.path(outDir, paste0("VE_cover_nMC=",nMC,"_N=",n,"_nBoot=",nBoot,"_truncateMarker=",truncateMarker,"_pi=",pi[i],".txt")), quote=FALSE, row.names=FALSE, col.names=FALSE)
}
