...
source("h:/SCHARP/Sanofi/manuscript_VEcurveMethod/code/VEcurveMC_myFunctions.R")
...
for (i in 1:length(pi)){
  inferenceVE <- lapply(1:nMC, function(seed, s1grid, trueVEcurve, n, beta, betaSizeT1, betaPowerT3, pi, truncateMarker, nBoot){ 
    getInferenceVE(s1grid, trueVEcurve, n, beta, betaSizeT1, betaPowerT3, pi, truncateMarker, seed, nBoot) }, s1grid=s1, trueVEcurve=trueVEcurve, n=n, beta=beta, 
  ...
  save(out, file=file.path(outDir, paste0("VE_coverSizePower_nMC=",nMC,"_N=",n,"_nBoot=",nBoot,"_truncateMarker=",truncateMarker,"_pi=",pi[i],".RData")))
}
